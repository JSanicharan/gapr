...
def stream_fields_to_fvg(fields: dict[str, str]) -> FVG:
    return FVG(
        direction=Direction(fields["direction"]),
        top=float(fields["top"]),
        bottom=float(fields["bottom"]),
...
class RedisConsumer:
    ...
    def read_all_fvgs(self, stream_name: str) -> list[FVG]:
        entries = self.client.xrange(stream_name, "-", "+")
        return [stream_fields_to_fvg(fields) for _, fields in entries]
